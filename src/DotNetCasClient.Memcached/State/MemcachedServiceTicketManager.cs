...
{
    ///<summary>
    /// An IServiceTicketManager implementation that relies on Memcached for service ticket
    ...
    public sealed class MemcachedServiceTicketManager : IServiceTicketManager
    {
        ...
        public CasAuthenticationTicket GetTicket(string serviceTicket)
        {
            ...
            var key = GetTicketKey(serviceTicket);

            return MemcachedClientManager.Instance.Get<CasAuthenticationTicket>(key);
        }
        ...
        public void InsertTicket(CasAuthenticationTicket casAuthenticationTicket, DateTime expiration)
        {
            ...
            var key = GetTicketKey(casAuthenticationTicket.ServiceTicket);

            MemcachedClientManager.Instance.Store(StoreMode.Set, key, casAuthenticationTicket, expiration);
        }
        ...
        public void UpdateTicketExpiration(CasAuthenticationTicket casAuthenticationTicket, DateTime newExpiration)
        {
            ...
            // FYI this method performs the Memcached 'set' command which basically performs an upsert.
        ...
        public void RevokeTicket(string serviceTicket)
        {
            ...
            MemcachedClientManager.Instance.Remove(key);
        }
        ...
        public bool ContainsTicket(string serviceTicket)
        {
            ...
            return MemcachedClientManager.Instance.TryGet(key, out ticket);
        }
        ...
        /// </param>
        /// <returns>
        /// The cache key associated with the corresponding service ticket
        /// </returns>
        /// <exception cref="ArgumentNullException">serviceTicket is null</exception>
        /// <exception cref="ArgumentException">serviceTicket is empty</exception>
        private static string GetTicketKey(string serviceTicket)
        {
             ...
             *
             * CAS service tickets may have a length greater than that (https://apereo.github.io/cas/5.3.x/protocol/CAS-Protocol-Specification.html#311-service-ticket-properties).
             *
             * Due to the key length limitation in Memcached we must store a shorter unique key.
             *
             * A SHA512 hash of the service ticket will be used as the Memcached key because it will be 128 characters in length.
             */
            ...
            {
                var hash = sha512Managed.ComputeHash(Encoding.UTF8.GetBytes(CACHE_TICKET_KEY_PREFIX + serviceTicket));
